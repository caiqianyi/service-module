...
public class SensitivewordFilter {
	...
	private Map<LocalWordType,Map> sensitiveWordMap = null;
	...
	public SensitivewordFilter(){
		...
		for(LocalWordType wt : wts){
			SensitiveWordInit swi = new SensitiveWordInit(wt);
			Map map = swi.getSensitiveWordMap();
			sensitiveWordMap.put(wt, map);
		}
		logger.debug("all={}",sensitiveWordMap);
	}
	...
	public Set<String> getSensitiveWord(String txt , int matchType){
		...
		for(int i = 0 ; i < txt.length() ; i++){
			...
			if(length > 0){    //存在,加入list中
				...
				String word = txt.substring(i, end);
	...
	public int CheckSensitiveWord(String txt,int beginIndex,int matchType){
		...
		LocalWordType[] wts = LocalWordType.values();
		for(LocalWordType wt : wts){
			Map nowMap = sensitiveWordMap.get(wt);
			boolean isBreak = false;
			for(int i = beginIndex; i < txt.length() ; i++){
				word = txt.charAt(i);
				nowMap = (Map) nowMap.get(word);     //获取指定key
				if(nowMap != null){     //存在，则判断是否为最后一个
					logger.debug("word={},i={},txt={},beginIndex={},isEnd={}",word,i,txt,beginIndex,nowMap.get("isEnd"));
	...
	public static void main(String[] args) {
		...
		String string = "法轮地方元角淫荡乳房八淡淡的九毛";
